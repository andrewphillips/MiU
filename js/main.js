...
// Variable defaults
	var packageCat = [":: Choose a Category ::", "Images", "Videos", "Maps :: Routes", "Favorite Places", "Epic Moments"];
	var	dontRushValues;
	var	rushValues;
	var	shipNotifyValue;
	var	bringValues;
...
*/		
	...
	function storeData(key){
		...
		getDontRushValue(); */
		var item = {};
			item.packagenick 			= ["Title:", $('packagenick').value];
			item.datesend 				= ["Date:", $('datesend').value];
			...
			item.numberpackages 	= ["Rating:", $('numberpackages').value];
		...
		alert("Item(s) added to scrapbook!");
	}
	...
	function validate(e){
		...
		if(getPackageNick.value === ""){
			var packageError = "Please enter a title for your event.";
		...
		if(getCategoryName.value === ":: Choose a Category ::"){
	...
	function getData(){
		...
		autoFillData();
		getRangeValue();
		alert("There is no data stored so default data was added.");
		}
